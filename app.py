...
def add_post(user_id):
    ...
    for key in request.form:
        if key[0:3] == "tag" and request.form[key] == "on":
            tags.append(Tag.query.get(key[3:]))
       
    new_post = Post(creator_id = user_id, title=title, content=content, tags=tags)
...
@app.route("/posts/<post_id>/edit", methods=["POST"])
def edit_tag(post_id):
    tag = Tag.query.get_or_404(post_id)
